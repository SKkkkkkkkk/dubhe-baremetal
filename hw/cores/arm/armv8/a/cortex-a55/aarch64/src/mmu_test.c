...
void config_mmu(void)
{
	...
	tt_l1[1] = TT_S1_NORMAL_WBWA | TT_S1_INNER_SHARED | 0x40000000; // [0x40000000, 0x80000000) // NORMAL_WBWA, INNER_SHARED, RW
	tt_l1[2] = TT_S1_NORMAL_WBWA | TT_S1_INNER_SHARED | 0x80000000; // [0x80000000, 0xC0000000) // NORMAL_WBWA, INNER_SHARED, RW
	tt_l1[3] = TT_S1_NORMAL_WBWA | TT_S1_INNER_SHARED | 0xC0000000; // [0xC0000000, 0x100000000) // NORMAL_WBWA, INNER_SHARED, RW
	...
	for(uint8_t i = 0; i<64 ; i++) // 256KB ROM, NORMAL_WBWA INNER_SHARED RO
		tt_l3_1[i] = 3 | TT_S1_NORMAL_WBWA | TT_S1_INNER_SHARED | TT_S1_PRIV_RO | (i*4096);

	for(uint8_t i = 0; i<64 ; i++) // 256KB SYSRAM, NORMAL_WBWA INNER_SHARED RW
		tt_l3_2[i] = 3 | TT_S1_NORMAL_WBWA | TT_S1_INNER_SHARED | (i*4096 + 0x20000000);
