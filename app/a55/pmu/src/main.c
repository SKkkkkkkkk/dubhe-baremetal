...
#define USE_GIC 1
    ...
    void pmu_irqhandler (void) {
        ...
        uint32_t exp_isr;
        printf("PMU interrupt occurred! gic_pass:%d",gic_pass);
...
int main()
{
  ...
  while (gic_pass < 2) {
    ...
    for(int i=0; i<17; i++) {
        ...
        if(result == golden) {
            #if DEBUG_PMU
            printf("OK: PWSR[0x%x]:0x%x", i, result);
            #endif
        } else {
            printf("ERROR!! PWSR[0x%x]:0x%x, exp:0x%x", i, result, golden);
            gic_pass = 0xff;
        }
        ...
        if(result == golden) {
            #if DEBUG_PMU
            printf("OK: CR[0x%x]:0x%x", i, result);
            #endif
        } else {
            printf("ERROR!! CR[0x%x]:0x%x, exp:0x%x", i, result, golden);
            gic_pass = 0xff;
        }
    }

    printf("gic_pass is %d", gic_pass);
    if(gic_pass == 2) {
        TEST_PASS;
    } else {
