				       ...
				       unsigned int *out_level)
{
	...
	for (unsigned int level = start_level;
		...
		if (idx >= entries) {
			printf("Missing xlat table entry at address 0x%lx\n",
			     virtual_addr);
			return NULL;
		}
		...
		if (desc_type == INVALID_DESC) {
			printf("Invalid entry (memory not mapped)\n");
			return NULL;
		}
				   ...
				   size_t size, uint32_t attr)
{
	...
	if (!IS_PAGE_ALIGNED(base_va)) {
		printf("%s: Address 0x%lx is not aligned on a page boundary.\n",
		     __func__, base_va);
		return -EINVAL;
	}
	...
	if ((size % PAGE_SIZE) != 0U) {
		printf("%s: Size 0x%zx is not a multiple of a page size.\n",
		     __func__, size);
		return -EINVAL;
	}

	if (((attr & MT_EXECUTE_NEVER) == 0U) && ((attr & MT_RW) != 0U)) {
		printf("%s: Mapping memory as read-write and executable not allowed.\n",
		     __func__);
		return -EINVAL;
	}
	...
	printf("Changing memory attributes of %zu pages starting from address 0x%lx...\n",
		pages_count, base_va);
	...
	for (unsigned int i = 0U; i < pages_count; ++i) {
		...
		if (((desc & DESC_MASK) != PAGE_DESC) ||
			(level != XLAT_TABLE_LEVEL_MAX)) {
			printf("Address 0x%lx is not mapped at the right granularity.\n",
			     base_va);
			printf("Granularity is 0x%lx, should be 0x%lx.\n",
		...
		if (attr_index == ATTR_DEVICE_INDEX) {
			if ((attr & MT_EXECUTE_NEVER) == 0U) {
				printf("Setting device memory as executable at address 0x%lx.",
				     base_va);
				return -EINVAL;
			}
		}
