...
static uint8_t axp2101_regaddrs[] = {
    [AXP210X_COMM_STAT1] = 0x01,  [AXP210X_REG_BROM] = 0xA1,
    [AXP210X_REG_MODE] = 0x17,    [AXP210X_REG_COMSTAT0] = 0x00,
    [AXP210X_REG_CONFIG] = 0xA2,  [AXP210X_REG_VBAT] = 0x34,
    [AXP210X_REG_TM] = 0x3C,      [AXP210X_REG_SOC] = 0xA4,
    [AXP210X_REG_T2E] = 0xA6,     [AXP210X_REG_T2F] = 0xA8,
    [AXP210X_REG_LOWSOC] = 0x1a,  [AXP210X_REG_IIN_LIM] = 0x16,
    [AXP210X_REG_ICC_CFG] = 0x62, [AXP210X_CHGLED_CFG] = 0x69,
    [AXP210X_COMM_STAT0] = 0x00,  [AXP210X_CHIP_ID] = 0x03,
...
static int axp210x_read_vbat(union power_supply_propval *val) {
    ...
    for (i = 0; i < 3; i++) {
        ret = axp210x_info->read(axp210x_info->regaddrs[ AXP210X_REG_VBAT ],
                                 data, 2);
        if (ret < 0) return ret;
...
static int axp210x_read_soc(union power_supply_propval *val) {
    ...
    ret =
        axp210x_info->read(axp210x_info->regaddrs[ AXP210X_REG_SOC ], data, 1);
    if (ret < 0) return ret;
        ...
        if (data[ 0 ] > axp210x_info->regcache.soc) {
            if (axp210x_info->regcache.soc < 92) {
                ...
                if (sysday.tv_sec - lasttime >
                    (SOC_RISE_INTERVAL * (axp210x_info->regcache.soc - 92))) {
                    axp210x_info->regcache.soc++;
                    val->intval = axp210x_info->regcache.soc;
                    lasttime    = sysday.tv_sec;
                    //					printk("no
...
static int axp210x_read_time2empty(union power_supply_propval *val) {
    ...
    for (i = 0; i < 3; i++) {
        ret = axp210x_info->read(axp210x_info->regaddrs[ AXP210X_REG_T2E ],
                                 data, 2);
        if (ret < 0) return ret;
...
static int axp210x_read_time2full(union power_supply_propval *val) {
    ...
    for (i = 0; i < 3; i++) {
        ret = axp210x_info->read(axp210x_info->regaddrs[ AXP210X_REG_T2F ],
                                 data, 2);
        if (ret < 0) return ret;
        ttemp[ i ] = ((data[ 0 ] << 0x08) | (data[ 1 ]));
    }
...
static int axp210x_reset_gauge(void) {
    ...
    ret =
        axp210x_info->read(axp210x_info->regaddrs[ AXP210X_REG_MODE ], data, 1);
    if (ret < 0) return ret;

    data[ 0 ] |= AXP210X_MODE_RSTMCU;
    ret = axp210x_info->write(axp210x_info->regaddrs[ AXP210X_REG_MODE ], data,
                              1);
    if (ret < 0) return ret;
...
int axp210x_model_update(void) {
    ...
    ret = axp210x_info->read(axp210x_info->regaddrs[ AXP210X_MODULE_EN ], data,
                             1);
    if (ret < 0) goto UPDATE_ERR;
    data[ 0 ] &= ~BIT(1);
    ret = axp210x_info->write(axp210x_info->regaddrs[ AXP210X_MODULE_EN ], data,
                              1);
    if (ret < 0) goto UPDATE_ERR;
    msleep(1000);
    ...
    for (i = 0; i < axp210x_info->data.model_size; i++) {
        ret = axp210x_info->write(axp210x_info->regaddrs[ AXP210X_REG_BROM ],
    ...
    for (i = 0; i < axp210x_info->data.model_size; i++) {
        ...
        if (para[ i ] != axp210x_info->data.model[ i ]) {
            axp210x_warn("model [%d] para reading %02x != write %02x\n", i,
                         para[ i ], axp210x_info->data.model[ i ]);
            ret = -EINVAL;
            //	goto UPDATE_ERR;
        }
    }
...
static bool axp210x_model_update_check(void) {
    ...
    ret = axp210x_info->read(axp210x_info->regaddrs[ AXP210X_REG_CONFIG ], data,
                             1);
    if (ret < 0) goto CHECK_ERR;
    if ((data[ 0 ] & AXP210X_CFG_UPDATE_MARK) == 0) goto CHECK_ERR;
...
static int axp210x_reg_update(void) {
    ...
    ret =
        axp210x_info->read(axp210x_info->regaddrs[ AXP210X_REG_VBAT ], data, 2);
    if (ret < 0) return ret;
    ...
    if (ret < 0) return ret;
    ret =
        axp210x_info->read(axp210x_info->regaddrs[ AXP210X_REG_T2E ], data, 2);
    if (ret < 0) return ret;
    axp210x_info->regcache.t2e = (data[ 0 ] << 8) + data[ 1 ];
    ret =
        axp210x_info->read(axp210x_info->regaddrs[ AXP210X_REG_T2F ], data, 2);
    if (ret < 0) return ret;
...
static int axp210x_get_bat_status(struct power_supply        *psy,
    ...
    ret = axp210x_info->read(di->regaddrs[ AXP210X_COMM_STAT1 ], data, 1);
    if (ret) {
...
static int axp210x_get_property(struct power_supply        *psy,
    ...
    case POWER_SUPPLY_PROP_CAPACITY:
        if (READ_ONCE(di->stat.bat_stat) && READ_ONCE(di->stat.bat_read))
...
static irqreturn_t axp210x_irq_handler_thread(int irq, void *data) {
    ...
    if (ret != 0) printk(KERN_ALERT "%s: soc update fail!\n", __FUNCTION__);
    ret = axp210x_read_vbat(&val);
    if (ret != 0) printk(KERN_ALERT "%s: vbat update fail!\n", __FUNCTION__);
...
static ssize_t register_write(struct class *class, struct class_attribute *attr,
    ...
    address = simple_strtoul(buf, &endptr, 16);
    start   = endptr + 1;
    if (start < buf + count) value = simple_strtoul(start, &endptr, 16);
...
static struct axp_interrupts axp_charger_irq[] = {
    ...
    [AXP2101_VIRQ_CHARGE_OVER]  = {"charge over", axp210x_irq_handler_thread},
    [AXP2101_VIRQ_LOW_WARNING1] = {"low warning1", axp210x_irq_handler_thread},
    [AXP2101_VIRQ_LOW_WARNING2] = {"low warning2", axp210x_irq_handler_thread},
...
int axp2101_charger_dt_parse(struct device_node     *node,
    ...
    axp_config->wakeup_usb_in  = of_property_read_bool(node, "wakeup_usb_in");
    axp_config->wakeup_usb_out = of_property_read_bool(node, "wakeup_usb_out");
    axp_config->wakeup_bat_in  = of_property_read_bool(node, "wakeup_bat_in");
    axp_config->wakeup_bat_out = of_property_read_bool(node, "wakeup_bat_out");
...
static void axp2101_parse_device_tree(struct axp210x_device_info *di) {
    ...
    if (!of_property_read_u32(di->dev->of_node, "pmu_pre_chg", &prop))
        regmap_update_bits(di->regmap, axp2101_IPRECHG_CFG, 0x0f, prop / 25);
    else
    ...
    if (!of_property_read_u32(di->dev->of_node, "pmu_iterm_limit", &prop)) {
        if (prop) {
            regmap_update_bits(di->regmap, axp2101_ITERM_CFG, 0x0f, prop / 25);
            regmap_update_bits(di->regmap, axp2101_ITERM_CFG, BIT(4), BIT(4));
        } else {
    ...
    if (!of_property_read_u32(di->dev->of_node, "pmu_chled_enable", &prop) &&
        !prop) {
        regmap_update_bits(di->regmap, axp2101_CHGLED_CFG, 0x01, 0x00);
    }

    prop = di->dts_info.pmu_usbpc_cur;
    if (!of_property_read_u32(di->dev->of_node, "iin_limit", &prop) || true) {
        for (i = 0; i < ARRAY_SIZE(iin_lim_tbl); i++) {
            if (prop < iin_lim_tbl[ i ]) break;
        }
        ...
        regmap_update_bits(di->regmap, axp2101_regaddrs[ AXP210X_REG_IIN_LIM ],
                           GENMASK(2, 0), i);
    }
    ...
    if (!of_property_read_u32(di->dev->of_node, "icc_cfg", &prop) || true) {
        ...
        regmap_update_bits(di->regmap, axp2101_regaddrs[ AXP210X_REG_ICC_CFG ],
                           GENMASK(4, 0), prop);
    }
    ...
    if (of_property_read_bool(di->dev->of_node, "pmu_btn_chg_en")) {
        regmap_update_bits(di->regmap, axp2101_MODULE_EN, BIT(2), BIT(2));
    } else {
...
static void battery_chk_online_v1(struct work_struct *work) {
    ...
    if (di->stat.bat_init == 0) {
        ...
        ret = axp210x_info->read(axp210x_info->regaddrs[ AXP210X_MODULE_EN ],
                                 data, 1);
        if (ret < 0) goto err_read;
    ...
    if ((!(data[ 0 ] & BIT(3))) && (di->stat.bat_stat != 0)) {
        ...
        axp210x_reset_gauge();
        s_chg = ktime_get();
        regmap_update_bits(di->regmap, axp2101_regaddrs[ AXP210X_CHGLED_CFG ],
                           BIT(0), 0);
        power_supply_changed(di->bat);
    }
...
static void battery_chk_online(struct work_struct *work) {
    ...
    if (rst[ cnt ] < AXP210X_VBAT_MIN) rst[ cnt ] = 0;
    cnt = ++cnt == 3 ? 0 : cnt;
    ...
    if (!rst[ 0 ] || !rst[ 1 ] || !rst[ 2 ]) {
        if (di->stat.bat_stat != 0) {
            ...
            s_chg = ktime_get();
            regmap_update_bits(
                di->regmap, axp2101_regaddrs[ AXP210X_CHGLED_CFG ], BIT(0), 0);
            power_supply_changed(di->bat);
        }
    } else {
        if (di->stat.bat_stat != 1) {
            ...
            s_chg = ktime_get();
            regmap_update_bits(
                di->regmap, axp2101_regaddrs[ AXP210X_CHGLED_CFG ], BIT(0), 1);
            power_supply_changed(di->bat);
        }
    }
...
static int axp2101_get_param(struct axp210x_device_info *di) {
    ...
    if (!pres) return -ENOMEM;

    n_para = of_parse_phandle(di->dev->of_node, "param", 0);
    if (!n_para) goto e_n_para;

    if (of_property_read_string(n_para, "select", &pparam)) goto e_para;

    r_para = of_get_child_by_name(n_para, pparam);
    if (!r_para) goto e_para;

    cnt = of_property_read_variable_u8_array(r_para, "parameter", pres, 1,
                                             AXP210X_MAX_PARAM);
    if (cnt <= 0) goto e_n_parameter;
...
static void axp2101_virq_dts_set(struct axp210x_device_info *di, bool enable) {
    ...
    if (!dts_info->wakeup_usb_in)
        axp2101_irq_set(axp_charger_irq[ AXP2101_VIRQ_USB_IN ].irq, enable);
    if (!dts_info->wakeup_usb_out)
        axp2101_irq_set(axp_charger_irq[ AXP2101_VIRQ_USB_OUT ].irq, enable);
    if (!dts_info->wakeup_bat_in)
        axp2101_irq_set(axp_charger_irq[ AXP2101_VIRQ_BAT_IN ].irq, enable);
    if (!dts_info->wakeup_bat_out)
        axp2101_irq_set(axp_charger_irq[ AXP2101_VIRQ_BAT_OUT ].irq, enable);
    if (!dts_info->wakeup_bat_charging)
        axp2101_irq_set(axp_charger_irq[ AXP2101_VIRQ_CHARGING ].irq, enable);
    ...
    if (!dts_info->wakeup_bat_untemp_work)
        axp2101_irq_set(axp_charger_irq[ AXP2101_VIRQ_BAT_UNTEMP_WORK ].irq,
                        enable);
    if (!dts_info->wakeup_bat_ovtemp_work)
        axp2101_irq_set(axp_charger_irq[ AXP2101_VIRQ_BAT_OVTEMP_WORK ].irq,
                        enable);
    if (!dts_info->wakeup_untemp_chg)
        axp2101_irq_set(axp_charger_irq[ AXP2101_VIRQ_BAT_UNTEMP_CHG ].irq,
                        enable);
    if (!dts_info->wakeup_ovtemp_chg)
        axp2101_irq_set(axp_charger_irq[ AXP2101_VIRQ_BAT_OVTEMP_CHG ].irq,
                        enable);
}
