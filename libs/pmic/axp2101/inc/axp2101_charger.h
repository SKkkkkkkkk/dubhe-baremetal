...
#define AXP_OF_PROP_READ(name, def_value) do {
}
while (0)
...
#define axp210x_alway(...) printf("[AXP210X]" __VA_ARGS__);

#define axp210x_err(...)                         \
    do {                                         \
        if (debug_level >= XPOWER_DBG_ERROR)     \
            printf("[AXP210X_ERR]" __VA_ARGS__); \
    } while (0)

#define axp210x_warn(...)                         \
    do {                                          \
        if (debug_level >= XPOWER_DBG_WARN)       \
            printf("[AXP210X_WARN]" __VA_ARGS__); \
    } while (0)

#define axp210x_info(...)                         \
    do {                                          \
        if (debug_level >= XPOWER_DBG_INFO)       \
            printf("[AXP210X_INFO]" __VA_ARGS__); \
    } while (0)

#define axp210x_debug(...)                       \
    do {                                         \
        if (debug_level >= XPOWER_DBG_DEBUG)     \
            printf("[AXP210X_DBG]" __VA_ARGS__); \
    } while (0)
#else
